...
Handle h_TimerLeviathanMangement[MAXPLAYERS+1] = {null, ...};
bool b_WeaponAccuracyModified[MAXENTITIES];
bool b_WeaponAttackspeedModified[MAXENTITIES];
float f_MLeviathanhuddelay[MAXPLAYERS+1];

void Leviathan_Map_Precache() //Anything that needs to be precaced like sounds or something.
{
	Zero(h_TimerMg42Mangement);
}

void Reset_stats_Leviathan_Singular_Weapon(int weapon) //This is on weapon remake. cannot set to 0 outright.
{
	b_WeaponAccuracyModified[weapon] = false;
	b_WeaponAttackspeedModified[weapon] = false;
}

public void Weapon_Leviathan_DoubleStrike(int client, int weapon, int StandingStill)
{
	float Accuracy = Attributes_Get(weapon, 106, 3.0);
...
public void Enable_MG42(int client, int weapon) // Enable management, handle weapons change but also delete the timer if the client have the max weapon
{
	if (h_TimerMg42Mangement[client] != INVALID_HANDLE)
	{
		...
		if(i_CustomWeaponEquipLogic[weapon] == WEAPON_LEVIATHAN) //6 Is for Passanger
		{
			//Is the weapon it again?
			//Yes?
			delete h_TimerMg42Mangement[client];
			h_TimerLeviathanMangement[client] = null;
			DataPack pack;
			h_TimerLeviathanMangement[client] = CreateDataTimer(0.25, Timer_Management_Leviathan, pack, TIMER_REPEAT);
	...
	if(i_CustomWeaponEquipLogic[weapon] == WEAPON_Leviathan) //6 is for MG42.
	{
		DataPack pack;
		h_TimerMg42Mangement[client] = CreateDataTimer(0.25, Timer_Management_Leviathan, pack, TIMER_REPEAT);
		pack.WriteCell(client);
		pack.WriteCell(EntIndexToEntRef(weapon));
	}
}
...
public Action Timer_Management_Leviathan(Handle timer, DataPack pack)
{
	pack.Reset();
	...
	Leviathan_Cooldown_Logic(client, weapon);
		
	return Plugin_Continue;
}


public void Leviathan_Cooldown_Logic(int client, int weapon)
{
	if(f_MLeviathanhuddelay[client] < GetGameTime())
	{
		...
		if(weapon_holding == weapon) //Only show if the weapon is actually in your hand right now.
		{
			...
			{
				PrintHintText(client,"Leviathan Accuracy maxed.");
			}
			else
			{
				PrintHintText(client,"Leviathan Inaccurate!!");
			}
